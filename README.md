...
- **爬升統計**：每天的累積爬升與下降（以 ±5 公尺門檻過濾 GPS 高度雜訊，10 公尺以內的來回小起伏不計入），整趟總爬升顯示於統計區塊與圖卡
- **地圖顯示**：MapLibre GL + OpenStreetMap 底圖，每天一個顏色，自動縮放到涵蓋軌跡與照片圖釘的範圍
- **分天摘要與統計**：每天與整趟的距離、天數與爬升；點開任一天可展開該天的高度剖面（里程–高度曲線，標出最高與最低高度）與該天的爬升、下降，斷訊切段處不連線，該天沒有高度資料時顯示說明文字
- **匯出 GPX**：GPX 1.1，每天一個 `<trk>`，每段一個 `<trkseg>`
- **下載圖卡**：2160×2160 PNG，可編輯標題，含日期範圍、天數、總距離、總爬升與每天顏色圖例（標籤為該天的月/日，例如 `3/12`）；有「含底圖」與「極簡」兩種風格
